...
public class LogicContractSetting {
    ...
    public interface IContractSettingListener {
        ...
        void onLeverageChange(int leverage, String text);
    }
    ...
    public void refreshLeverage(int leverage, String text){
        for (int i = 0; i<mListeners.size(); i++){
            if (mListeners.get(i) != null){
                mListeners.get(i).onLeverageChange(leverage, text);
            }
        }
    }
    ...
    private static int s_trigger_price_type = 0;
    ...
    public static int getTriggerPriceType(Context context) {
        if (s_trigger_price_type_first) {
            s_trigger_price_type = PreferenceManager.getInstance(context).getSharedInt(PreferenceManager.PREF_TRIGGER_PRICE_TYPE, 0);
