            ...
            if (file != null) {
                if (file.isFile() && file.exists()) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                        uri = FileProvider.getUriForFile(mContext, mActivity.getPackageName()+ShareToolUtil.AUTHORITY, file);
                    } else {
...
//            intent.setAction(Intent.ACTION_SEND_MULTIPLE);// 分享多张图片时使用
            ...
            if (picFile != null) {
                if (picFile.isFile() && picFile.exists()) {
                    Uri uri;
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                        uri = FileProvider.getUriForFile(mActivity, mActivity.getPackageName()+ShareToolUtil.AUTHORITY, picFile);
                    } else {
