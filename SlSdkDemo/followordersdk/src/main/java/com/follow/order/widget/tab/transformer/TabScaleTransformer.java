package com.follow.order.widget.tab.transformer;
...
public class TabScaleTransformer implements ViewPager.PageTransformer {
    ...
    public TabScaleTransformer(SlidingScaleTabLayout slidingScaleTabLayout, PagerAdapter pagerAdapter,
                               float textSelectSize, float textUnSelectSize, int fixPosition) {
        this.slidingScaleTabLayout = slidingScaleTabLayout;
        this.pagerAdapter = pagerAdapter;
        this.textSelectSize = textSelectSize;
        this.textUnSelectSize = textUnSelectSize;
...
//        if(slidingScaleTabLayout.getViewPager() == null){
//            return;
//        }
        int tabPosition = pagerAdapter.getItemPosition(view);
        final TextView currentTab = slidingScaleTabLayout.getTitle(tabPosition);
//        final TextView currentTab = slidingScaleTabLayout.getTitle(slidingScaleTabLayout.getViewPager().indexOfChild(view));
        if (currentTab == null) {
            return;
        }
        if (tabPosition == fixPosition) {
            currentTab.post(new Runnable() {
                @Override
                public void run() {
                    currentTab.setTextSize(TypedValue.COMPLEX_UNIT_PX, textSelectSize);
                }
            });
            // 回调设置的页面切换效果设置
